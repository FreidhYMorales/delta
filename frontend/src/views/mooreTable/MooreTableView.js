// Moore state-table view — mirrors `mealyTable/MealyTableView.js`'s shape
// (alphabet-override field, add/delete-state buttons, editable name cell
// doubling as the initial-marker) but for the Moore document: no accepting
// column/marker, columns are the INPUT alphabet only and hold plain target
// labels (no output pairing — Moore's output isn't per-transition), plus a
// dedicated "Salida" column with no Mealy equivalent. Lives inside Moore's
...
export class MooreTableView {
  /**
   * @param {HTMLElement} container
   * @param {import('../../store/MooreDocStore.js').MooreDocStore} docStore
   * @param {import('../../commands/MooreContext.js').MooreContext} ctx renaming
  ...
  _build() {
    ...
    this.root.className = "table-view moore-table-view";
    ...
    alphabetLabel.setAttribute("for", "moore-table-alphabet-input");
    ...
    this.alphabetInput.id = "moore-table-alphabet-input";
  ...
  _render() {
    ...
    headRow.append(
      ...
      thWith("Salida", "La salida que este estado emite al ser alcanzado"),
    );
    ...
    for (const state of states) {
      ...
      outputInput.className = "table-output-input";
      outputInput.value = state.output ?? "";
      outputInput.addEventListener("change", () => {
        this._lastEditPromise = this._onOutputCellEdit(state.id, outputInput.value);
      });

      tr.append(tdWith(deleteCheckbox, "table-col-narrow"), thWith(nameInput), tdWith(outputInput));
      ...
      for (const symbol of columns) {
        ...
        input.title = "estado destino, p.ej. q1";
   ...
   * `MooreDoc::set_initial` is a single `Option<StateId>` slot that silently
   * replaces whoever held it before, same as Mealy's table.
  ...
  /** @param {number} id @param {string} raw */
  async _onOutputCellEdit(id, raw) {
    const state = this.docStore.getState(id);
    if (!state) return;
    const output = parseOutputCell(raw);
    if (output !== (state.output ?? null)) {
      await this.docStore.apply([{ op: "SetOutput", state: id, output }]);
    }
  }

  /** Resolve an existing state by label, or create one — same "auto-create,
   * reuse by label" convention as the FA/Mealy tables' `_resolveOrCreateId`. */
  ...
  async _onCellEdit(fromId, symbol, raw, edgesFromStateAtRenderTime) {
    const labels = parseCellEntries(raw);
    const desired = [];
    let index = 0;
    for (const label of labels) {
      desired.push(await this._resolveOrCreateId(label, index));
      index++;
    }
