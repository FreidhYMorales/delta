// TM (Turing Machine) canvas — same core interaction model as
// `PdaDiagramView.js` (4-tool L0 pattern: select/create-state/create-
// transition/delete, click-drag, pan/zoom, keyboard dispatch, right-click
// context menu — all driven by `tmRegistry.js`, design D6's "nothing
// bypasses the registry" rule). Reuses `views/diagram/geometry.js`'s pure
// curve/layout math directly, same as PDA/Mealy/Moore's views — NOT
// duplicated here.
//
// Differences from `PdaDiagramView.js`, all traceable to `TmDoc` itself:
//  - Accepting-state double circle (`.state-accepting-ring`) — reused
//    verbatim, same as PDA's.
//  - Transitions are a FLAT list keyed by `id`, each carrying
//    `tapes: {read,write,direction}[]` instead of PDA's single
//    `(input, pop, push)` triple. Same multiple-transitions-between-the-
//    same-pair fan-out as PDA's (every transition gets its own real,
//    independently-clickable curved arc via `curvedEdgePath`/`selfLoopPath`
//    with a growing offset/angle per index within its `(from,to)`/self-loop
//    group — see `pdaDiagram/PdaDiagramView.js`'s header comment for the
//    real-JFLAP-vs-this-project rationale, identical here).
//  - Creating/editing a transition prompts once PER TAPE in sequence
//    (`promptTransitionTapes` from `tmRegistry.js`), not a fixed three
//    times — tape count is dynamic (`tmLogic.js`'s `effectiveTapeCount`).
//  - "Abrir"/"Guardar" live here for the same reason as PDA's — no menu bar
//    of TM's own (see docs/decisions.md).

import { curvedEdgePath, nextStateLabel, preferredLoopAngle, selfLoopPath } from "../diagram/geometry.js";
import { findTmAction, findTmActionByKeybinding, keybindingOf, promptTransitionTapes } from "../../commands/tmRegistry.js";
import { formatTransitionLabel } from "./tmLogic.js";
...
export class TmDiagramView {
  ...
  _buildStaticDom() {
    ...
    this.root.className = "diagram-view tm-diagram-view";
    ...
    // No menu bar of TM's own yet (see docs/decisions.md) — Abrir/Guardar
    // live here instead, next to the status chip, native-JSON only (no
    // `.jff` for TM), same as PDA/Mealy/Moore's views.
  ...
  /** Single input-dispatch path — mirrors `PdaDiagramView._dispatchKey`. */
  _dispatchKey(event) {
    const action = findTmActionByKeybinding(keybindingOf(event));
   ...
   * `PdaDiagramView._onCanvasContextMenu`. */
   ...
   * convention as `PdaDiagramView`'s. Runs the per-tape prompt flow
   * (`promptTransitionTapes`, `tmRegistry.js`) once both endpoints are
   * picked; cancelling any tape's prompt aborts with no op applied. */
  async _handleCreateTransitionClick(stateId) {
    ...
    this._pendingFrom = null;
    const result = await promptTransitionTapes(this.ctx);
    if (!result) return;
  ...
  _renderCanvas() {
    ...
    // data model itself stays flat (`TmTransitionView`s keyed by id), this
    ...
    for (const state of states) {
      if (state.initial) {
        ...
        arrow.setAttribute("marker-end", "url(#tm-arrow)");
   ...
   * (same "hit path" convention as `PdaDiagramView`'s). Double-click opens
   * `transition.edit`. */
  _renderTransitionArc(t, d, labelX, labelY, isSelfLoop) {
    const el = document.createElementNS(SVG_NS, "path");
    el.dataset.transitionId = String(t.id);
    el.setAttribute("marker-end", "url(#tm-arrow)");
    el.setAttribute("d", d);
    ...
    hit.addEventListener("dblclick", () => {
      ...
      this._lastEditPromise = findTmAction("transition.edit").run(this.ctx);
    });

    const label = document.createElementNS(SVG_NS, "text");
    label.textContent = formatTransitionLabel(t.tapes);
  ...
  _renderInspector() {
    ...
    if (sel.kind === "state") {
      ...
      this.inspector.textContent = t ? `Transición ${t.from} -> ${t.to}: ${formatTransitionLabel(t.tapes)}` : "Sin selección";
    }
  }
  ...
  _renderStatusBar() {
    ...
    this.statusBar.textContent =
      `TM · ${derived.deterministic ? "determinista" : "no determinista"} · Q=${states.length} · ` +
      `Σ=${derived.alphabet.length} · Cintas=${derived.tape_count} · δ=${transitions.length}` +
  ...
  _renderCanvasInfoBar() {
    ...
    this.canvasStatusChip.textContent = `TM · ${states.length} ${noun}`;
