...
import { PdaFormalView } from "./PdaFormalView.js";
...
async function setup() {
  ...
  const view = new PdaFormalView(container, docStore);
...
describe("PdaFormalView", () => {
  it("renders into a .formal-view.pda-formal-view wrapper", async () => {
    const { container } = await setup();
    expect(container.querySelector(".formal-view.pda-formal-view")).toBeTruthy();
    expect(container.querySelector("textarea")).toBeTruthy();
  });

  it("renders Q/Sigma/Gamma/q0/F and one delta(from,input,pop)=(to,push) line per transition", async () => {
    const { container } = await setup();
    const textarea = container.querySelector("textarea");
    expect(textarea.value).toContain("Q = {q0, q1}");
    expect(textarea.value).toContain("F = {q1}");
    expect(textarea.value).toContain("delta(q0, a, Z) = (q1, A Z)");
  });

  it("shows a validation error and does not mutate the document on an invalid edit", async () => {
    const { container, client } = await setup();
    const textarea = container.querySelector("textarea");
    textarea.value = "Q = {q0}\ndelta(q0, a, Z) = (q9, Z)"; // q9 undeclared
  ...
  it("applies a valid edit as a sequence of ops against the DocStore", async () => {
    ...
    const textarea = container.querySelector("textarea");
    textarea.value = ["Q = {q0, q1}", "q0 = q1", "F = {q1}", "delta(q0, a, Z) = (q1, A Z)"].join("\n");
    container.querySelector("button.apply").dispatchEvent(new MouseEvent("click", { bubbles: true }));
    await view._lastApplyPromise;

    const allOps = client.pdaApply.mock.calls.flatMap((c) => c[0]);
  ...
  it("adding a second transition between an already-connected pair through formal text keeps both", async () => {
    ...
    expect(allOps).toEqual(
      expect.arrayContaining([{ op: "AddTransition", from: 1, to: 2, input: "b", pop: [], push: [] }]),
    );
