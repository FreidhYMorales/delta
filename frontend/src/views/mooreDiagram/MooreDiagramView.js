// Moore machine canvas — same core interaction model as `MealyDiagramView.js`
// (4-tool L0 pattern: select/create-state/create-transition/delete,
// click-drag, pan/zoom, keyboard dispatch, right-click context menu — all
// driven by `mooreRegistry.js`, design D6's "nothing bypasses the registry"
// rule). Reuses `views/diagram/geometry.js`'s pure curve/layout math
// directly, same as Mealy's view.
//
// Differences from `MealyDiagramView.js`, all traceable to `MooreDoc` itself:
//  - No accepting-state double circle, no `state.toggleAccepting` context-
//    menu item — Moore has no accepting states, same as Mealy.
//  - Edge labels show plain input symbols (`formatInputs`), not
//    `input/output` pairs — Moore's output lives on the STATE, so each
//    state gets a second, smaller label under its name showing its output.
//  - Creating a transition prompts once for a single input symbol
//    (`ctx.promptInput`, `parseInputPrompt`), not an "input/output" pair.
//  - A new context-menu item, `state.setOutput`, has no Mealy equivalent.
//  - "Abrir"/"Guardar" live here for the same reason as Mealy's — no menu
//    bar of Moore's own (see docs/decisions.md).
...
import { findMooreAction, findMooreActionByKeybinding, keybindingOf } from "../../commands/mooreRegistry.js";
...
const OUTPUT_LABEL_OFFSET = 14;

export class MooreDiagramView {
  /**
  ...
  _buildStaticDom() {
    ...
    this.root.className = "diagram-view moore-diagram-view";
    ...
    // No menu bar of Moore's own yet (see docs/decisions.md) — Abrir/Guardar
    // live here instead, next to the status chip, native-JSON only (no
    // `.jff` for Moore), same as Mealy's view.
    ...
    marker.setAttribute("id", "moore-arrow");
  ...
  /** Single input-dispatch path — mirrors `MealyDiagramView._dispatchKey`. */
  _dispatchKey(event) {
    const action = findMooreActionByKeybinding(keybindingOf(event));
    ...
    switch (this.ctx.activeTool) {
      ...
      case "delete":
        if (stateId != null) {
          ...
          const [from, to] = edgeKey.split(":").map(Number);
          this.docStore.apply([{ op: "SetTransitions", from, to, inputs: [] }]);
        }
        break;
   ...
   * `MealyDiagramView._onCanvasContextMenu`, with `state.setOutput` added
   * (no Mealy equivalent). */
   ...
   * convention as `MealyDiagramView`'s. Prompts for a single input symbol
   * (`ctx.promptInput`), not an "input/output" pair — Moore's output lives
   * on the state, set separately via `state.setOutput`. */
  async _handleCreateTransitionClick(stateId) {
    ...
    const text = await this.ctx.promptInput(existing ? formatInputs(existing.inputs) : "");
    const input = parseInputPrompt(text);
    if (!input) return;
    const inputs = mergeInput(existing?.inputs ?? [], input);
    await this.docStore.apply([{ op: "SetTransitions", from, to: stateId, inputs }]);
  }
  ...
  _renderCanvas() {
    ...
    for (const edge of edges) {
      ...
      el.setAttribute("marker-end", "url(#moore-arrow)");
      if (d) {
        el.setAttribute("d", d);
      ...
      label.textContent = formatInputs(edge.inputs);
    ...
    for (const state of states) {
      if (state.initial) {
        ...
        arrow.setAttribute("marker-end", "url(#moore-arrow)");
      ...
      // Second, smaller label under the state showing its output — no
      // equivalent in `MealyDiagramView` (Mealy's output is per-edge, not
      // per-state). Only rendered when an output is actually set, so an
      // empty/unset state doesn't show a dangling blank line under it.
      if (state.output) {
        const outputLabel = document.createElementNS(SVG_NS, "text");
        outputLabel.textContent = `/ ${state.output}`;
        outputLabel.setAttribute("x", String(state.x));
        outputLabel.setAttribute("y", String(state.y + STATE_RADIUS + OUTPUT_LABEL_OFFSET));
        outputLabel.setAttribute("class", "state-output-label");
        this.svg.appendChild(outputLabel);
      }
    }
  }
  ...
  _renderInspector() {
    ...
    if (sel.kind === "state") {
      ...
      this.inspector.textContent =
        `Estado ${state.label} (#${state.id})` +
        (state.initial ? " · inicial" : "") +
        (state.output ? ` · salida: ${state.output}` : "");
    } else if (sel.kind === "edge") {
      const edge = this.docStore.getEdge(sel.from, sel.to);
      this.inspector.textContent = edge
        ? `Transición ${sel.from} -> ${sel.to}: ${formatInputs(edge.inputs)}`
        : "Sin selección";
    }
  }
  ...
  _renderStatusBar() {
    const states = this.docStore.getStates();
    const edges = this.docStore.getEdges();
    const transitionCount = edges.reduce((sum, e) => sum + e.inputs.length, 0);
    const derived = this.docStore.derived;
    this.statusBar.textContent =
      `Moore · ${derived.deterministic ? "determinista" : "no determinista"} · Q=${states.length} · ` +
  ...
  _renderCanvasInfoBar() {
    ...
    this.canvasStatusChip.textContent = `Moore · ${states.length} ${noun}`;
