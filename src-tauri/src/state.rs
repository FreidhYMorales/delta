...
//!
//! `MealySession`/`MooreSession` are each a genuinely separate piece of
//! managed state, not a variant of `Session` — same "isolated, not a
//! generalization" choice as `MealyDoc`/`MooreDoc` vs `FaDoc`
//! (docs/decisions.md). All three are always managed (`.manage(...)` in
