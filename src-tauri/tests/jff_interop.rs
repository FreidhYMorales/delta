...
fn jff_import_replaces_the_session_document_and_returns_a_report() {
    ...
    let result = jff::import(&session, fixture("dfa.jff")).expect("import must succeed");
...
fn jff_import_rejects_non_fa_types_with_a_clear_error_and_leaves_session_unchanged() {
    ...
    let err = jff::import(&session, fixture("reject_pda.jff")).unwrap_err();
    assert!(err.contains("pda") || err.to_lowercase().contains("unsupported"));

    let snap = app_lib::commands::doc::snapshot(&session);
...
fn jff_export_writes_a_reopenable_file_and_returns_a_report() {
    let session = Session::new();
    jff::import(&session, fixture("dfa.jff")).unwrap();

    let dir = std::env::temp_dir();
    let path = dir.join(format!("jflap-jff-export-test-{}.jff", std::process::id()));
    let path_str = path.to_string_lossy().to_string();

    let report = jff::export(&session, path_str.clone()).expect("export must succeed");
    assert_eq!(report.direction, "Export");

    let session2 = Session::new();
    let reimported = jff::import(&session2, path_str.clone()).expect("exported file must re-import");
