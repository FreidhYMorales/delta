...
fn apply_reports_exactly_the_expected_patches_and_bumps_the_revision() {
    ...
    let result = mealy::apply(&session, ops(&[MealyEditOpDto::AddState { label: "q0".into(), x: 1.0, y: 2.0 }])).unwrap();
...
fn snapshot_reflects_states_transitions_and_derived_facts_after_apply() {
    ...
    .unwrap();
    let snap = mealy::snapshot(&session);
...
fn undo_reverses_the_most_recent_apply_and_redo_reapplies_it() {
    let session = MealySession::new();
    mealy::apply(&session, ops(&[MealyEditOpDto::AddState { label: "q0".into(), x: 0.0, y: 0.0 }])).unwrap();
    assert_eq!(mealy::snapshot(&session).states.len(), 1);

    let undone = mealy::undo(&session).expect("there is a transaction to undo");
    assert_eq!(undone.revision, 2);
    assert!(matches!(&undone.patches[0], MealyDocPatch::StateRemoved { .. }));
    assert_eq!(mealy::snapshot(&session).states.len(), 0);

    let redone = mealy::redo(&session).expect("there is a transaction to redo");
...
fn undo_on_a_fresh_session_is_none() {
    let session = MealySession::new();
    assert!(mealy::undo(&session).is_none());
}

#[test]
fn save_then_open_round_trips_the_document() {
    let session = MealySession::new();
    mealy::apply(&session, ops(&[MealyEditOpDto::AddState { label: "q0".into(), x: 3.0, y: 4.0 }])).unwrap();
    let path = std::env::temp_dir().join(format!("mealy-ipc-test-{}.json", std::process::id()));
    let path_str = path.to_str().unwrap().to_string();

    mealy::save(&session, path_str.clone()).unwrap();

    let fresh_session = MealySession::new();
    let loaded = mealy::open(&fresh_session, path_str).unwrap();
...
fn sim_runs_the_current_document_and_reports_the_output() {
    ...
    .unwrap();
    let q0 = mealy::snapshot(&session).states.iter().find(|s| s.label == "q0").unwrap().id;
    let q1 = mealy::snapshot(&session).states.iter().find(|s| s.label == "q1").unwrap().id;
    mealy::apply(
        &session,
        ops(&[
            MealyEditOpDto::SetInitial { id: Some(q0) },
    ...
    let outcome = mealy::sim(&session, vec!["a".to_string()]);
